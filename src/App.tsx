...
function App() {
  ...
  const sidebarItems = useMemo(() => {
    ...
    sorted.forEach((topic, i) => {
      ...
      items.push({ key: topic.id, label: topic.label, parentId: topic.id, rank: i + 1, barWidth, isChild: false })
  ...
  useEffect(() => {
    ...
    if (!scrollContainer || sidebarItems.length === 0) return
    const container: HTMLDivElement = scrollContainer
    let frameId: number
    let pos = 0
    let paused = false
              ...
              >
                ...
                <span
                  ...
                  style={{
                    ...
                    background: `rgba(0, 255, 136, ${item.isChild ? 0.22 : 0.30 + (1 - item.rank / sidebarItems.filter(x => !x.isChild).length) * 0.45})`,
                  }}
                />
              </button>
            ))}
