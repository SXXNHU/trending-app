...
exports.computeOrbitPositions = computeOrbitPositions;
...
const SECTION_BY_TOPIC_ID = new Map(SECTION_LAYOUTS.flatMap((section) => section.topicIds.map((topicId) => [topicId, section.id])));
...
function midpointAngle(from, to) {
    return wrapAngle(from + shortestAngleDelta(from, to) * 0.5);
}
function clampAngleToSection(angle, centerAngle, span) {
    const halfSpan = span / 2;
    const delta = shortestAngleDelta(centerAngle, angle);
    if (delta > halfSpan)
        return wrapAngle(centerAngle + halfSpan);
    if (delta < -halfSpan)
        return wrapAngle(centerAngle - halfSpan);
    return wrapAngle(angle);
}
function getSectionOffset(angle, startAngle) {
    const delta = shortestAngleDelta(startAngle, angle);
    return delta < 0 ? delta + Math.PI * 2 : delta;
}
function buildLargestGapSlots(anchorOffsets, usableSpan, count) {
    const slots = [];
    const segments = [];
...
function getBaseAngle(topic, index) {
    return wrapAngle(topic.angle + index * 0.12);
}
function buildNeighborMap(topics) {
    return new Map(topics.map((topic) => [
        topic.id,
        new Set((topic.links ?? []).filter((linkedId) => linkedId !== topic.id)),
    ]));
}
function resolveSectionAssignments(topics, baseAngles, neighborMap) {
    const assignments = new Map();
    topics.forEach((topic) => {
        const explicitSection = SECTION_BY_TOPIC_ID.get(topic.id);
        if (explicitSection)
            assignments.set(topic.id, explicitSection);
    });
    topics.forEach((topic) => {
        if (assignments.has(topic.id))
            return;
        ...
        SECTION_LAYOUTS.forEach((section) => {
            ...
            neighbors?.forEach((neighborId) => {
                if (SECTION_BY_TOPIC_ID.get(neighborId) === section.id)
                    score += 1;
            });
            section.topicIds.forEach((sectionTopicId) => {
                if ((neighborMap.get(sectionTopicId)?.has(topic.id) ?? false))
                    score += 0.6;
            });
...
function buildSectionLayout(topics) {
    const baseAngles = new Map(topics.map((topic, index) => [topic.id, getBaseAngle(topic, index)]));
    ...
    const resolvedAngles = new Map();
    const orbitOverrides = new Map();
...
function computeOrbitPositions(topics) {
    ...
    topics.forEach((topic, index) => {
        ...
        const y = Math.sin(angle * 1.45 + orbitIndex * 0.8) * VERTICAL_VARIANCE
            + (orbitIndex - 1) * 0.9
